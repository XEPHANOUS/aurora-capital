...
export const AGENT_DEFINITIONS: Record<AgentType, Omit<Agent, 'reputation' | 'status'>> = {
  ...
  analyst: {
    id: 'analyst',
    name: 'Analista',
    description: 'Correlaciona señales de contexto, técnico e inversión',
    agentType: 'hybrid',
    priority: 75,
    influence: 78,
    reportsTo: 'supervisor',
...
function getAgentVoteReason(agentType: AgentType, positive: boolean): string {
  const reasons: Record<AgentType, { positive: string; negative: string }> = {
    ...
    analyst: {
      positive: 'Correlación positiva entre señales de agentes',
      negative: 'Contradicciones entre señales de agentes',
    },
    risk: {
...
export function generateDecisionSession(
  ...
  if (survivalAgent) {
    ...
    const survivalRecommendation: 'approve' | 'neutral' | 'veto' = wouldBreachReserve ? 'veto' : survivalStatus === 'safe' ? 'approve' : 'neutral';
  ...
  if (directorAgent) {
    const dirRec: DirectorAgentRecommendation = {
      ...
      decisionAction: finalDecision === 'approved' ? action : 'HOLD',
  ...
  const riskMetrics = calculateRiskMetrics(amount, 50000, 3.5);
  ...
  const minConsensus = 60;
  const minConfidence = 50;
  const dailyLossExceeded = survivalMetrics.maxDrawdown > config.dailyLossLimit;
  ...
  if (hasVeto) {
    ...
    blockReason = 'Agente de Supervivencia ha ejercido su derecho de veto';
  } else if (dailyLossExceeded) {
    executionBlocked = true;
    blockReason = 'Límite de pérdida diaria excedido';
  } else if (consensusLevel < minConsensus) {
    executionBlocked = true;
    blockReason = `Consenso insuficiente (${consensusLevel.toFixed(0)}% < ${minConsensus}%)`;
  } else if (combinedConfidence < minConfidence) {
    executionBlocked = true;
    blockReason = `Confianza insuficiente (${combinedConfidence.toFixed(0)}% < ${minConfidence}%)`;
  }
  
  return {
    id: `session-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
    timestamp: timestamp.toISOString(),
    status: executionBlocked ? 'rejected' : hasVeto ? 'vetoed' : status,
    proposal: {
      asset,
      action,
      amount
    ...
    finalDecision: status !== 'active' ? {
      ...
      reason: executionBlocked ? blockReason : explanation,
      timestamp: new Date(timestamp.getTime() + 8000).toISOString(),
      executionBlocked,
      blockReason: executionBlocked ? blockReason : undefined
    } : undefined,
    consensusLevel,
...
export function generateMockDecisionSessions(agents: Agent[], config: SystemConfig, currentCapital: number, count: number = 5): DecisionSession[] {
  ...
  const statuses: ('completed' | 'vetoed' | 'rejected')[] = ['completed', 'completed', 'completed', 'vetoed', 'rejected'];
